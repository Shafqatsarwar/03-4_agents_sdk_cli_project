...
async def main():
    async with MCPClient(
        ...
        tool_result = await _client.call_tool("read_doc", {"doc_id": "deposition.md"})
        print("Tool results!:", tool_result)


        # Example of calling a tool
        if tools:
            tool_name = tools[0].name
            tool_input = {"example_param": "value"}
            result = await _client.call_tool(tool_name, tool_input)
            print(f"Result from {tool_name}:", result)

        # Example of reading a resource
        resource_content = await _client.read_resource("example/resource")
